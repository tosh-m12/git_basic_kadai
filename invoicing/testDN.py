...
def extract_data(engine, ids):
    try:
        ...
        query = f"""
            SELECT
                dn.id, fe.company_name AS customer_company_name, dn.bill_from, dn.bill_to, sto, whs, svc, trs, dsb, oth, dn.cn_vat6, dn.cn_vat9,
                rate_date, rate, sub_total_cny, sub_total_usd, iv_amount_cny, iv_amount_usd,
                d_issued, dn.c_code, dn.ac_term, serial_num, dn.b_cat,
                COALESCE(rl.add1, rl_null.add1) AS add1,
                COALESCE(rl.add2, rl_null.add2) AS add2,
                COALESCE(rl.pic, rl_null.pic) AS pic,
                COALESCE(rl.mr_ms, rl_null.mr_ms) AS mr_ms,
                COALESCE(rl.tel, rl_null.tel) AS tel,
                COALESCE(rl.company_name_full, rl_null.company_name_full) AS company_name_full
            FROM dnmasterlist dn
            JOIN f_extract fe ON dn.c_code = fe.c_code
            LEFT JOIN recipientlist rl ON dn.c_code = rl.c_code AND rl.b_cat = dn.b_cat
            LEFT JOIN recipientlist rl_null ON dn.c_code = rl_null.c_code AND (rl_null.b_cat IS NULL OR rl_null.b_cat = '')
    ...
    try:
        if not dataframe.empty:
            for idx, row in dataframe.iterrows():
                ...
                ac_term_b_cat = f"{row['ac_term']}{row['b_cat']}"
                
                
                company_folder = os.path.join(base_output_path, ac_term_b_cat, sanitized_company_name)
