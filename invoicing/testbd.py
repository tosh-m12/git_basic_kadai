...
def extract_ids_from_bd_mexico(engine):
    try:
        query = f"SELECT f_extract_id, rev, b_cat FROM {lookup_table} WHERE `new` = '*' AND grand_total <> 0"
        df = pd.read_sql(query, engine)
        print("IDs extracted from bd_mexico")
        return df[['f_extract_id', 'rev', 'b_cat']].to_dict(orient='records')
...
def create_invoice(customer, rev_suffix, template_path, b_cat):
    try:
        ...
        # Set immex fields based on b_cat
        if b_cat == 'A':
            ws['E26'] = customer['labelling']
            ws['E27'] = customer['immex_en']
            ws['G28'] = customer['immex_vex']
        elif b_cat == 'C':
            ws['E26'] = customer['immex_en']
            ws['G27'] = customer['immex_vex']
            ws['E28'] = customer['labelling']
...
# Extract f_extract_id, rev, and b_cat from bd_mexico where new is "*"
...
if id_rev_list:
    ...
    b_cats = {item['f_extract_id']: item['b_cat'] for item in id_rev_list}
    ...
    if data is not None:
        for index, customer in data.iterrows():
            ...
            b_cat = b_cats[customer['id']]
            if b_cat == 'A':
                template_path = template_invoice_a
            elif b_cat == 'C':
                template_path = template_invoice_c
            else:
                print(f"Invalid billing category: {b_cat} for customer {customer['company_name']}")
                continue

            create_invoice(customer, rev_suffix, template_path, b_cat)
